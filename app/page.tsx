...
export default function Page() {
  ...
  // ✅ canvas ref nằm trong Dialog để bắn pháo hoa
  const presentCanvasRef = useRef<HTMLCanvasElement>(null);

  // ✅ fire confetti đúng nơi (Dialog thì bắn lên canvas trong Dialog)
  const fire = (options: confetti.Options) => {
    const canvas = presentOpen ? presentCanvasRef.current : null;
    if (canvas) {
  ...
  const bigFireworks = () => {
    try {
      fire({ particleCount: 340, spread: 95, origin: { y: 0.6 } });
      fire({ particleCount: 200, spread: 130, origin: { y: 0.2 } });
      fire({ particleCount: 240, spread: 80, origin: { y: 0.8 } });
    } catch {}
  };
